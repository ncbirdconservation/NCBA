...
```{r}
some_checklists <- head(get_all_checklists(ncba_config=config, 
                                           drop_ncba_col=TRUE), 200)
```

Plot the checklists
```{r}
footprints <- records_as_sf_dev(some_checklists, kind="checklists", 
                                method="point-radius")
plot(footprints$geometry)
```
