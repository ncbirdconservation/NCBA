...
ggplot() +
  ...
  ggtitle("Blocks That Need Observations")
```
...
ggplot() +
  ...
  ggtitle("Blocks Needing Summer Observations and Highest Reported Breeding Category")
```
...
needed_sf <- blocks_needed(species = species, source = "GAP", season = "winter",
                           database = database, NCBA_only = NCBA_only,
                           observations = obs) 
...
ggplot() +
  geom_sf(data = blocks_sf) +
  geom_sf(data = needed_sf, aes(fill = needed)) + 
  ggtitle("Blocks for Which Winter Observations are Needed")
```
...
ggplot() +
  geom_sf(data = blocks_sf) +
  geom_sf(data = needed_sf, aes(fill = needed)) + 
  ggtitle("Blocks for Which Summer Observations are Needed (GAP)")
```
...
ggplot() +
  geom_sf(data = blocks_sf) +
  geom_sf(data = needed_sf, aes(fill = needed)) + 
  ggtitle("Blocks for Which Winter Observations are Needed (eBird)")
```
...
ggplot() +
  geom_sf(data = blocks_sf) +
  geom_sf(data = needed_sf, aes(fill = needed)) + 
  ggtitle("Blocks for Which Winter Observations are Needed (GAP)")
```
```{r}
# Get a dataframe of blocks that need observations
needed_sf <- blocks_needed(species = "Bald Eagle", 
...
ggplot() +
  geom_sf(data = blocks_sf) +
  geom_sf(data = needed_sf, aes(fill = needed)) + 
  ggtitle("Blocks for Which Breeding Observations are Needed (eBird)")
```
