...
get_observations_dev <- function(species, dataset = "AtlasCache", 
  ...
  #   the NCBA mongodb.
  # ncba_only -- TRUE or FALSE whether to restrict records to NCBA records or not.
  ...
  if (dataset == "AtlasCache") {
    ...
    if (is.null(fields) == TRUE) {
      ...
      # Add "OBSERVATIONS" to the field list, it is needed.
      fields <- c(fields, "OBSERVATIONS")
      
      # Convert the list of field names to a mongolite filter string
      fields_string <- paste0('{', paste0('"', fields, '" : true', 
                                        collapse = ', '), '}')
      
      nc_data <- connection_ebd$find(query, fields = fields_string) %>%
...
```{r}
fields <- c("common_name", "global_unique_identifier", "project_code",
            "ncba_block")
...
There should only be one species name in the data frame
```{r}
