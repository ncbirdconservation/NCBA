...
ggplot(data=by_locality) +
  geom_boxplot(mapping=aes(y=count, x=""), color="darkblue", 
               outlier.colour="green", show.legend=TRUE) + 
  coord_flip() + 
...
method_A <- checklists %>%
  ...
  select(BLOCK_NAME, checklists) %>%
  # Join back with blocks spatial frame to fill in zeros (as a data frame)
  data.frame() %>%
  select(-c(geometry)) %>%
  right_join(blocks_sf, by=("BLOCK_NAME" = "BLOCK_NAME")) %>%
  select(BLOCK_NAME, checklists, geometry) %>%
...
sample_from <- c("SOUTHPORT", "FUNSTON", "WINNABOW", "LELAND", "CURRIE",
...
method_B <- buffered_coordinates %>%
  ...
  right_join(blocks_sf, by="BLOCK_NAME") %>%
  select(BLOCK_NAME, checklists, geometry) %>%
  replace_na(list(checklists=0)) %>%
  st_as_sf()  
...
priority_centroids <- st_centroid(filter(blocks_sf, TYPE == "Priority")$geometry)
...
Note: checklist footprints with center coordinates within select blocks are included in the map in order to display a sample of theunderlying data for method B.  Priority centroids identify priority atlas blocks.
...
print("method A")
summary(count_AB$method_A)
print("method B")
summary(count_AB$method_B)
```
...
priority_blocks <- blocks_sf %>%
  ...
  left_join(data.frame(count_AB), by=("BLOCK_NAME" = "BLOCK_NAME"))
  
ggplot(data=priority_blocks, aes(ysn_checklists)) +
  geom_bar() +
  ylab("Priority Blocks") +
  xlab("") + 
  labs(title="Many priority blocks remain unsampled",
