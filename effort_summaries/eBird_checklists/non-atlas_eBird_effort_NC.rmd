---
title: "Non-atlas eBird Effort in North Carolina"
author: "N.M. Tarr"
...
* There are currently `r comma(length(unique(checklists$checklist_id)))` atlas checklists from North Carolina in eBird.
...
ggplot(data=by_week) +
  ...
  labs(title="Non-atlaser activity varies greatly throughout the year",
...
ggplot(data=by_locality) +
  ...
  coord_flip() + 
  labs(title="Non-atlasers heavily sampled some locations",
       caption="Checklists from before 2021 are not included") +
  ylab("Checklists (n)") + 
...
ggplot(data=locality_bins, aes(x="", y=value, fill=times_surveyed)) +
  ...
  theme_void() + 
  labs(title="Most locations sampled by non-atlasers have only been sampled once",
...
ggplot(data=by_locality_type, aes(x="", y=count, fill=locality_type)) +
  ...
  scale_fill_viridis_d(alpha = 1, option="D") +
  theme_void() + 
  labs(title="Non-atlasers assigned many checklists to hotspots, but most are not",
       caption="Checklists from before 2021 are not represented")
...
knitr::kable(most_visited, caption="Locations visited 100 or more times by non-atlasers.")
...
ggplot(data=by_county) +
  geom_boxplot(mapping=aes(y=count, x=""), color="darkred", 
               outlier.colour="orange", show.legend=TRUE) + 
  coord_flip() + 
  labs(title="A handful of counties have many more non-atlas checklists than others",
...
ggplot(data=priority_blocks, aes(ysn_checklists)) +
  ...
  xlab("") + 
  labs(title="Many priority blocks were unsampled by non-atlasers",
...
ggplot(data=core_observers) +
  geom_boxplot(mapping=aes(y=count, x=""), color="darkblue", 
               outlier.colour="orange", show.legend=TRUE) + 
  coord_flip() + 
  labs(title='Active, non-atlasing eBirders have generally submitted < 100 checklists',
       caption="Checklists from before 2015 were not included in this summary") +
  ylab("Checklists (n)") + 
  scale_y_continuous(n.breaks=12)
```
