...
# function definition in blocks.R
  ...
  # convert counts in spp_acc to percentages # ** not run; using counts below ***
  ...
  spp_acc$c2_4 <- apply(spp_acc[,4:6],1,sum)	# 'coded spp' = sum of C2, C3, C4 
  # y position ofhorizontal line for 25% threshold of 'coded spp'	(with codes)
  ...
  plot_response <- ggplot(data=spp_acc,aes((min), all)) +
	# ggplot(data=spp_acc,aes(0, all)) +
	# ggplot(data=spp_acc,aes((min), all)) + ylim(0,max(spp_acc$all)) +
    ...
    geom_smooth(method = 'loess', se=F, formula = 'y ~ x',	aes(y = c2_4, color="Coded Species")) +
    geom_smooth(method = 'loess', se=F, formula = 'y ~ x',	aes(y = c4, color="Confirmed")) +
    geom_smooth(method = 'loess', se=F, formula = 'y ~ x',	aes(y = c3, color="Probable")) +
    geom_smooth(method = 'loess', se=F, formula = 'y ~ x',	aes(y = c2, color="Possible")) +
    ...
    geom_text(aes(0,coded_spp_25perc),label = "25% goal", vjust=-0.5, hjust=-0.05) +
    geom_text(aes(0,55),label = "Minimum coded species", vjust=-0.5, hjust=-0.05) +
    xlim(0,obs_min) +
...
ggplot(data=spp_acc,aes((min), all)) + ylim(0,max(spp_acc$all)) +
    geom_hline( yintercept=(55),linetype=2) +			# min coded spp
    geom_hline( yintercept=(coded_spp_25perc),linetype=2) +	# 25% of coded spp
    geom_smooth(method = 'loess', se=F, formula = 'y ~ x',	aes(y = all, color="All Species")) +
    geom_smooth(method = 'loess', se=F, formula = 'y ~ x',	aes(y = c2_4, color="Coded Species")) +
    geom_smooth(method = 'loess', se=F, formula = 'y ~ x',	aes(y = c4, color="Confirmed")) +
    geom_smooth(method = 'loess', se=F, formula = 'y ~ x',	aes(y = c3, color="Probable")) +
    geom_smooth(method = 'loess', se=F, formula = 'y ~ x',	aes(y = c2, color="Possible")) +
    scale_colour_manual( name="Breeding status", # values = c("#ff1a1a", "#ffbf00", "#ccccff")) +
	values = c("#aaaaaa", "#2a3b4d", "#ff1a1a", "#ffbf00", "#ccccff")) +
	# values = c("#444444", "#2a3b4d", "#ff1a1a", "#ffbf00", "#ccccff")) +
    geom_text(aes(0,coded_spp_25perc),label = "25% goal", vjust=-0.5, hjust=-0.05) +
    geom_text(aes(0,55),label = "Minimum coded species", vjust=-0.5, hjust=-0.05) +
    xlim(0,obs_min) +
...
###
ggplot(data=spp_acc,aes((min), all)) + 
    geom_hline( yintercept=(55),linetype=2) +			# min coded spp
    geom_hline( yintercept=(coded_spp_25perc),linetype=2) +	# 25% of coded spp
    geom_smooth(method = 'loess', se=F, formula = 'y ~ x', aes(y = all, color="a")) +
    geom_smooth(method = 'loess', se=F, formula = 'y ~ x', aes(y = c2_4, color="b")) +
    geom_smooth(method = 'loess', se=F, formula = 'y ~ x', aes(y = c4, color="c")) +
    geom_smooth(method = 'loess', se=F, formula = 'y ~ x', aes(y = c3, color="d")) +
    geom_smooth(method = 'loess', se=F, formula = 'y ~ x', aes(y = c2, color="e")) +
    ...
    geom_text(aes(0,coded_spp_25perc),label="25% goal", vjust=-0.5, hjust=+0.20) +
    geom_text(aes(0,55),label = "Minimum coded species", vjust=-0.5, hjust=+0.10) +
    xlim(0,obs_min) + ylim(0,max(spp_acc$all)) +
    ylab("Count of Species") + xlab("Observation Time") 
...
###   The 'span' option reduces the amount of line smoothing by 'loess'
spn <- 0.25
ggplot(data=spp_acc,aes((min), all)) + 
    geom_hline( yintercept=(55),linetype=2) +			# min coded spp
    geom_hline( yintercept=(coded_spp_25perc),linetype=2) +	# 25% of coded spp
    geom_smooth(method = 'loess', se=F, formula='y~x', span=spn, aes(y = all, color="a")) +
    geom_smooth(method = 'loess', se=F, formula='y~x', span=spn, aes(y = c2_4, color="b")) +
    geom_smooth(method = 'loess', se=F, formula='y~x', span=spn, aes(y = c4, color="c")) +
    geom_smooth(method = 'loess', se=F, formula='y~x', span=spn, aes(y = c3, color="d")) +
    geom_smooth(method = 'loess', se=F, formula='y~x', span=spn, aes(y = c2, color="e")) +
    scale_colour_manual( name="Breeding status", 
	values = c("a"="#aaaaaa", "b"="#2a3b4d", "c"="#ff1a1a", "d"="#ffbf00", "e"="#ccccff"),
	labels = c("All Species","Coded Species", "Confirmed", "Probable", "Possible")) +
    geom_text(aes(0,coded_spp_25perc),label="25% goal", vjust=-0.5, hjust=+0.20) +
