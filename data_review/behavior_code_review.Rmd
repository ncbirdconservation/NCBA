...
pe <- get_breeding_records(behaviors = c("PE")) %>%
  to_EBD_format() %>%
  auk_unique() %>%
  select(sampling_event_identifier, common_name, protocol_type, has_media) %>%
...
```{r}
box <- breeding_boxplot(species = NULL, data = pe, type = "interactive")
show(box)
```
...
The "get_breeding_records" function can retrieve records of "DD".
```{r}
...
The records can also be summarized and plotted by day of year with a boxplot that also provides hyperlinks for each record.
```{r}
box <- breeding_boxplot(species = NULL, data = df, type = "interactive")
show(box)
```